...
void plot_ascii(const std::vector<double>& x, const std::vector<double>& y_true, 
    ...
    double y_min = std::min({*std::min_element(y_true.begin(), y_true.end()),
                           *std::min_element(y_pred.begin(), y_pred.end())}) - 0.1;
    double y_max = std::max({*std::max_element(y_true.begin(), y_true.end()),
                           *std::max_element(y_pred.begin(), y_pred.end())}) + 0.1;
    
    std::cout << "\n=== Gaussian Function Learning with RBF Kernel GP ===" << std::endl;
...
double gaussian_function(double x, double amplitude = 1.0, double mean = 0.0, double std_dev = 1.0) {
    return amplitude * std::exp(-0.5 * std::pow((x - mean) / std_dev, 2));
}

int main() {
    try {
        // Generate training data from Gaussian function in range [-3, 3]
        ...
        const double std_dev = 1.2;
        
        for (int i = 0; i < n_train; ++i) {
            double x = -3.0 + (double)i / (n_train - 1) * 6.0; // Points from -3 to 3
            x_train_vec.push_back(x);
            y_train_vec.push_back(gaussian_function(x, amplitude, mean, std_dev));
        }
        ...
        std::unique_ptr<KernelBase> kernel(new RBFKernel(1.5, 0.7));
        GaussianProcess gp(std::move(kernel), 1e-6);
        
        std::cout << "Training Gaussian Process with RBF kernel..." << std::endl;
        std::cout << "Training data: " << n_train << " points from Gaussian function in [-3, 3]" << std::endl;
        std::cout << "Gaussian parameters: amplitude=" << amplitude << ", mean=" << mean 
        ...
        for (int i = 0; i < n_test; ++i) {
            double x = -4.0 + (double)i / (n_test - 1) * 8.0; // Range [-4, 4] for extrapolation
            x_test_vec.push_back(x);
            y_true_vec.push_back(gaussian_function(x, amplitude, mean, std_dev));
        }
        ...
        std::cout << "\nSuccess! GP learned the Gaussian function pattern." << std::endl;
        return 0;
