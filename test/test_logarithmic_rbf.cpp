...
void plot_ascii(const std::vector<double>& x, const std::vector<double>& y_true, 
    ...
    std::cout << "\n=== Logarithmic Function Learning with RBF Kernel GP ===" << std::endl;
...
int main() {
    try {
        // Generate training data from logarithmic function in range [0.1, 3]
        const int n_train = 8;
        std::vector<double> x_train_vec;
        std::vector<double> y_train_vec;
        
        for (int i = 0; i < n_train; ++i) {
            double x = 0.1 + (double)i / (n_train - 1) * 2.9; // Points from 0.1 to 3
            x_train_vec.push_back(x);
            y_train_vec.push_back(std::log(x)); // natural logarithm function
        }
        ...
        std::cout << "Training data: " << n_train << " points from logarithmic function in [0.1, 3]" << std::endl;
        ...
        for (int i = 0; i < n_test; ++i) {
            double x = 0.05 + (double)i / (n_test - 1) * 3.95; // Range [0.05, 4] for extrapolation
            x_test_vec.push_back(x);
            y_true_vec.push_back(std::log(x));
        }
        ...
        std::cout << "\nSuccess! GP learned the logarithmic function pattern." << std::endl;
        return 0;
