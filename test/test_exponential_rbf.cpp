...
void plot_ascii(const std::vector<double>& x, const std::vector<double>& y_true, 
    ...
    std::cout << "\n=== Exponential Function Learning with RBF Kernel GP ===" << std::endl;
...
int main() {
    try {
        // Generate training data from exponential function in range [0, 2]
        const int n_train = 8;
        std::vector<double> x_train_vec;
        std::vector<double> y_train_vec;
        
        for (int i = 0; i < n_train; ++i) {
            double x = (double)i / (n_train - 1) * 2.0; // Points from 0 to 2
            x_train_vec.push_back(x);
            y_train_vec.push_back(std::exp(x)); // exponential function
        }
        ...
        // Create GP with RBF kernel (smaller lengthscale for exponential)
        std::unique_ptr<KernelBase> kernel(new RBFKernel(2.0, 0.5));
        GaussianProcess gp(std::move(kernel), 1e-6);
        
        std::cout << "Training Gaussian Process with RBF kernel..." << std::endl;
        std::cout << "Training data: " << n_train << " points from exponential function in [0, 2]" << std::endl;
        ...
        for (int i = 0; i < n_test; ++i) {
            double x = -0.5 + (double)i / (n_test - 1) * 3.0; // Range [-0.5, 2.5] for extrapolation
            x_test_vec.push_back(x);
            y_true_vec.push_back(std::exp(x));
        }
        ...
        std::cout << "\nSuccess! GP learned the exponential function pattern." << std::endl;
        return 0;
