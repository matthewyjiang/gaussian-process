...
void plot_ascii(const std::vector<double>& x, const std::vector<double>& y_true, 
    ...
    std::cout << "\n=== Sine Function Learning with RBF Kernel GP ===" << std::endl;
...
int main() {
    try {
        // Generate training data from sine function in range [0, 1]
        const int n_train = 8;
        std::vector<double> x_train_vec;
        std::vector<double> y_train_vec;
        
        for (int i = 0; i < n_train; ++i) {
            double x = (double)i / (n_train - 1); // Points from 0 to 1
            x_train_vec.push_back(x);
            y_train_vec.push_back(std::sin(2 * M_PI * x)); // sine function
        }
        ...
        std::unique_ptr<KernelBase> kernel(new RBFKernel(1.0, 0.3));
        GaussianProcess gp(std::move(kernel), 1e-6);
        
        std::cout << "Training Gaussian Process with RBF kernel..." << std::endl;
        std::cout << "Training data: " << n_train << " points from sine function in [0, 1]" << std::endl;
        ...
        for (int i = 0; i < n_test; ++i) {
            double x = -0.2 + (double)i / (n_test - 1) * 1.4; // Range [-0.2, 1.2] for extrapolation
            x_test_vec.push_back(x);
            y_true_vec.push_back(std::sin(2 * M_PI * x));
        }
        ...
        std::cout << "\nSuccess! GP learned the sine function pattern." << std::endl;
        return 0;
